...
public class OrderServiceImplV1 implements OrderService{
	 ...
	 * 
	 *  Exception 처리를 직접하지 않고 이 method를 호출한 곳에 대리 요청한다(Exception을 던진다)
	 */
	
	/*
	 * insert를 호출하는 곳에서는 OrderVO에 주문정보 데이터를 담고
	 * 매겨변수로 전달해야 하며
	 * try - catch로 Exception처리를 하고 결과값으로 정수값을 받아서
	 * insert 유무를 처리해야 한다.
	 */
	@Override
	public int insert(OrderVO orderVO) throws SQLException {
		...
		// VO를 통해서 전달받은 데이터를 SQL 명령문과 합성하기
	 ...
	 * exception 처리를 수행해야 하며 return 값을
	 * List<OrderVO> 형 변수에 받아서 처리한다.
	 */
	@Override
	...
	/*
	 * selectAll() 과 findBySeq() method에서 ResultSet으로부터 데이터를
	 * getter하여 OrderVO에 setter하는 부분을 별도 method로 분리하고
	...
	public OrderVO findBySeq(long seq) throws SQLException {
		...
		// rSet 담기게 될 table의 데이터는 단 1개의 데이터만 담기거나, 없는 상태가 된다.
		ResultSet rSet = pSt.executeQuery();
		if(rSet.next()) {
			OrderVO orderVO = this.setOrderVO(rSet);
			return orderVO;			
