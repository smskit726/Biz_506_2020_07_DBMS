...
public class OrderInput {
	...
	public boolean orderInsert() {
		...
		if(ret) {
			if(oService.insert(orderVO)>0) {
				...
				System.out.println("주문서 등록(추가) 실패 :(");
			}
		}
		return ret;
	}
	...
	public boolean orderInput(OrderVO orderVO) {
		...
		System.out.printf("주문번호(%s, QUIT:종료) >> ", orderVO.getO_num() == null ? "NEW" : orderVO.getO_num());
		...
		while (true) {
			System.out.printf("고객번호(%s, QUIT:종료) >> ", orderVO.getO_cnum() == null ? "NEW" : orderVO.getO_cnum());
		...
		while (true) {
			System.out.printf("상품코드(%s, QUIT:종료) >> ", orderVO.getO_pcode() == null ? "NEW" : orderVO.getO_pcode());
		...
		while (true) {
			System.out.printf("상품가격(%d, QUIT:종료) >> ", orderVO.getO_price());
		...
		while (true) {
			System.out.printf("주문수량(%d, QUIT:종료) >> ", orderVO.getO_qty());
