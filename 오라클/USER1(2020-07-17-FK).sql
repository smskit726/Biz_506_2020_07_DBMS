...
외래키 (FOREIGN KEY)
==========================================================================================================================
두개(이상)의 테이블간에 Replation을 강제로 설정하여 테이블에 추가되는 데이터의 무결성을 보증하는 제약조건 설정
...
FK는 테이블을 생성할때 만들기도 하지만 일반적으로 테이블을 생성한 후 테이블 변경을 통해 만드는 것이 좋다.

가장 좋은 경우는 테이블을 생성하고 데이터를 한개도 추가하지 않은 상태로 설정하는 것이다
하지만 App을 사용하기 시작하는 초기단계에서는 다량의 데이터를 추가해야하는 일들이 많아서 이 때 FK가 설정이 되어 있으면,
Sub Table에 데이터를 추가하는데 많은 애로사항이 발생한다. 이런 경우 초기애 FK를 설정하지 않거나, FK 제거하고 데이터를 입력(추가)
하도록 하는 경우가 많다.

현재 스키마에 있는 학생정보와 성적정보 테이블을 FK(외래키) 관계로 설정을 한다.
FK 관계의 테이블을 부모테이블과 자식 테이블로 분류를 하는데, FK 설정은 자식테이블에서 실시한다.
...
-- 20200 학생의 국어점수는 학적정보가 이상이 없는 학생의 성적인지 확신할 수 있는 방법이 없다.
-- 따라서 이 데이터는 무결성원칙에 위배되는 데이터 일 수 있다.
...
==========================================================================================================================
    학생정보 st_num 데이터                 성적정보 st_num 데이터
         값이 추가(있다)           >>        있다(추가할 수 있다)
         값이 없다                 >>        절대 없다(추가할 수 없다)
         있어야 한다               <<        있더라(추가되어 있다)


만약 학생정보 데이터가 필요가 없어서 삭제를 할 경우 
=> 먼저 성적정보에서 삭제하고자 하는 학번의 성적데이터를 모두 삭제해야한다.
...
만약 저장된 데이터가 매우 많다면 상당한 시간이 소요될 것이고, 그 동안 Table의 데이터를 필요로하는 App은 실행이 중단된다. 
칼럼추가, 칼럼삭제, 칼럼명 변경, 칼럼타입 변경 등을 수행할 수 있다.
...
-- 삭제되는 키값의 데이터가 자식 테이블에 있으면
-- 같이 삭제하라
ON DELETE CASCADE; --(SET NULL);

-- 만약 부모테이블에서 키 값이 변경이 되면 자식테이블의 모든데이터를 찾아서 같은 값으로 바꿔라
-- ON UPDATE CASCADE; (Dead lock상태(교착상태)를 유발할 수 있다.)
...
테이블간의 데이터가 1:N 인경우) N상태의 테이블에 설정을 한다.
                                REFERENCES : 1 상태의 Table에 설정을 한다
테이블간의 데이터가 1:1 인경우) Work Table에 설정을 한다.
                                REFERENCES : Master Table에 설정을 한다.


학생정보, 학과 정보 간의 FK 설정하려고 한다.
학과정보는 학과명(코드)이 유일한 값으로 정해져 있따. 같은 이름의 학과는 존재할 수 없다.
학생정보는 학생마다 학과가 다르거나 같을 수 있어서 수 없이 많은 중복데이터가 있다
부모테이블 : 학과 1인 경우
자식테이블 : 학생정보 중의 학과는 N인 경우가 된다.
*/
