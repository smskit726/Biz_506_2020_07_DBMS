...
이 회사는 기존에 엑셀을 이용하여 거래처별 매입매출 관ㄹ리를 수행해 왔는데
최근 App 을 개발하여 데이터베이스화 하고 관리를 수행하려 한다.
엑셀데이터를 받아서 DB로 보내기 위해 변환 작업을 수행하고 있다.

엑셀 매입매출원장을 DB로 만들기 위해 매입, 매출을 분리하고, 날짜를 문자열화 했다.
이 데이터를 DB에 저장하려고 봤더니, 이 데이터에 PK칼럼으로 지정할만한 칼럼을 찾을 수가 없다.
이러한 경우(Work DB인 경우)에는 실제 데이터와 별도로 임의의 PK칼럼을 생성해서 관리 해 주는 것이 좋다.
*/
...
-- 이 데이터를 단가, 금액을 수정하기 위해 동일한 상품 데이터가 있는지 확인작업
...
-- WHERE io_pname = '투니스' 와 같이 지정하면 혹시 다른 매입, 매출 데이터가 있을 경우 막대한 문제를 일으킬 수 있다.
...
/*
- 원본데이터
수량, 매입단가, 매출단가, 매입금액, 매출금액 으로 칼럼을 분리하여 사용해왔다.
이러한 형태의 데이터는 각 칼럼에 NULL 값이 존재하여, 다양한 연산을 수행할 때 문제를 일으킬 수 있다.

- 변환된 데이터
수량, 단가, 금액 형식으로 칼럼을 통합작업을 수행했다.
칼럼으로 분리된 값들을 공통 칼럼을 선정하고 데이터를 통합하는 것을 보편적으로 제3정규화가 되었다 라고 한다.

변환된 데이터를 참조하여 매입과 매출을 구분하여 보고싶다 할 때,
...
-- 거래처별로 매입과 매출을 구분하여 볼 수 있는 Query를 완성했다.
-- 거래처별로 매입과 매출의 합계를 계산해 보고 싶다.
-- 이 데이터(일년간 거래내역)에서 거래처별로 총 매입금액, 총 매출금액을 계산해보고 싶다.

/*
1. 거래처별로 거래내역이 여러번 반복이 이루어 졌으므로 거래처명이 여러번 나타날 것이다.
   이 데이터를 거래처별로 묶어주어야 한다.
   GROUP BY io_dname
   
2. 전체 매출금액과 매입금액을 표시하는 가상칼럼(매출금액, 매입금액을 계산함)을 통계 함수로 묶어준다.
*/
SELECT io_dname,
