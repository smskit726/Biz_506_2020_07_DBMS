...
-- tbl_student의 st_grade 칼럼에 저장된 값 중
-- 최댓값(가장 큰값), 최솟값(가장 적은값)을 구하고 싶다.
...
학과 이름이 같은 이름들이 다수 존재하는데, 이들 이름을 1개씩만 추출하여 리스트로
보여준다면, 어떠한 학과가 있는지 알기 쉬울 것 같다.
...
-- st_name 칼럼에 저장된 값들이 중복된 값이 없어 GROUP BY가 의미가 없는 경우도 있다.
SELECT st_name FROM tbl_student
GROUP BY st_name;

-- 데이터를 학과이름으로 그룹 짓고 각 학과에 소속된 학생들이 몇 명씩인가를 알고싶다.
-- st_dept 칼럼으로 GROUP을 만들고 각 그룹에 속하는 레코드가 몇개인가를 카운트하여 보여준다.
SELECT st_dept, COUNT(*) FROM tbl_student
GROUP BY st_dept;

-- 학년별로 소속된 학생이 몇명인가 알고싶다.
-- st_grade 칼럼으로 GROUP을 만들고 각 그룹에 속하는 레코드가 몇개인가를 카운트하여 보여준다.
...
-- 단, 가장나이가 많은 학생의 이름, 또는 가장나이가 적은 학생의 이름을 찾는데는
-- 이 SQL로는 어렵다!
SELECT st_dept, MAX(st_age), MIN(st_age), AVG(st_age) FROM tbl_student
GROUP BY st_dept;
