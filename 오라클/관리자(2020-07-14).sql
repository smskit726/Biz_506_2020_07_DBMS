...
/*
현재 오라클에 접속된 사용자(sys)가 관리하는 table들이 있는데 그 중에서 tab이라는
이름의 tabale정보를 가져와서 나에게 보여달라!

오라클의 tab table은 현재 사용자가 관리하는 DB Object(객체)들의 정보를
보관하고 있는 table
*/
SELECT *
FROM tab;
...
DBMS가 보관하고 있는 데이터를 table 형식으로 보여달라 라는 명령
...
SIZE : 오라클에서는 성능의 효율성을 주기위해 일단 빈 공간을 일정부분 설정하게 된다.
    ...
    오라클 xe(express Edition)에서는 table Space의 최대 크개를 11G로 제한한다.
    만약 Size를 10G로 지정하고, 용량이 초과되어 AUTO NEXT로 추가가 되는 경우
    전체 Size가 11G를 넘어서면 오류가 나면서 더이상 데이터를 저장할 수 없게 된다.
    
AUTO...NEXT : 만약 초기에 지정한 SIZE공간에 데이터가 가득차면 자동으로 용량을 늘려서
    저장할수 있도록 만들어라
    
SIZE의 1M : 기본크기를 1024*1024 byte 크기로 지정하라.
            SIZE를 지정할 때 1MB라고 하지 않는다.
NEXT 500K : 자동으로 확장(늘리기)를 1024*500 크기로 설정
            NEXT를 지정할 때 500KB라고 하지 안는다.
...
변수, 값, 내용 등은 소문자로 사용하며 특별히 대소문자를 구분해야 하는 경우는
    별도 공지를 할 것
...
CREATE USER user1 IDENTIFIED BY 1234  -- 사용자 ID를 user1으로 설정하고 초기 비밀번호를 1234로 설정
DEFAULT TABLESPACE myTS;


-- DCL(Data Control Language)
-- 새로 생성된 user1에게 권한을 부여하자
GRANT CONNECT TO user1;  -- user1이 로그인만 할 수 있도록 권한(역할)을 부여
REVOKE CONNECT FROM user1;  -- user1이 로그인 할 수 있는 권한을 제거


-- user1이 로그인을 수행하고, 최소한으로 데이터들을 관리할 수 있도록 권한을 부여
/*
RESOURCE : 오라클에서 USER에게 줄 수 있는 권한 중 상당히 많은 일을 수행할 수 있는 권한
        현재 시스템에 설치된 모든 TableSpace를 대상으로 무제한(TableSpace가 허용하는범위)
        저장이 가능한 권한
