...
-- 만약 결과가 1개의 Record만 보일지라도 결과는 리스트(다수)형으로 출력된다(취급한다).
SELECT * FROM tbl_student
...
-- IN 연산자를 사용할 수 있다.
SELECT * FROM tbl_student
WHERE st_num IN ('20001', '20020', '20030');

-- 1개의 칼럼 값을 범위로 제한하는 SELECTION을 할 때
-- 부등호연산은 숫자 칼럼에만 적용되는 것이 원칙
-- 문자열일경우에도 저장된 데이터의 길이가 모두 같고 패턴이 같으면
-- 문자열도 부등호 연산이 가능하다.
SELECT * FROM tbl_student
...
SELECT sc_num, sc_kor, sc_eng, sc_math, sc_music, sc_art,
        ...
        -- ROUND(값) : 소수점이하 반올림하여 정수로 표현
        -- ROUND(값, 자릿수) : (자릿수 +1) 에서 반올림하여 자릿수 자리까지 소수점 표현
        ROUND( ( sc_kor + sc_eng + sc_math + sc_music + sc_art )/5,2 ) AS 평균,
        
        -- 무조건 자르기(절사)
        -- TRUNK(값) : 소수점 이하 모두 버리기
...
-- st_dept 칼럼 값이 같으면 그 범위(그룹, 파티션)내에서 st_name을 기준으로 내림차순 정렬하라
SELECT *
FROM tbl_student
WHERE st_num BETWEEN '20001' AND '20010'
ORDER BY st_dept, st_name DESC;

-- st_dept 칼럼의 값이 같은 Record끼리 묶고 묶인 레코드의 개수를 세어라
-- 부분합 연산
...
GROUP BY SUBSTR(sc_num,1,4)
ORDER BY 반; -- ORDER BY 는 SELECT 명령문의 절들 중에 가장 끝에 나타나야 한다.
