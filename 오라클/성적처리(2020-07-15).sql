...
PRIMARY KEY : 중복되지않고, NULL값이 아닌 값만 유효
              UNIQUE 와 NOT NULL을 포함한 제약조건이 설정
...
SQL 오류: ORA-00947: not enough values ->
    ...
    ex ) 나열된 칼럼 개수 6개인데, VALUES 데이터는 6개 미만으로 설정된 경우
*/
...
/*
- INSERT 수행 후 자주 발생하는 오류들
ORA-01400: cannot insert NULL into ("GRADE"."TBL_STUDENT"."ST_NAME") ->
...
ORA-01722: invalid number ->
    ...
    만약 NUMBER로 설정된 칼럼에 숫자를 '3' 다옴표로 묶어서 지정하면
    내부에서 문자열 -> 숫자형식으로 변환하여 저장한다. (자동형변환)
    그런데 문자열 -> 숫자형식으로 자동형변환을 하는데 숫자로 변환할 수 없는
    문자열이 포함되면 invalid nuber오류를 나타낸다.
...
ORA-02290: check constraint (GRADE.SYS_C007008) violated ->
    CHECK로 유효성 검사를 설정했는데 범위, Valid를 통과하지 못했을 때 발생하는 오류
