-- 여기는 grade 사용자 화면입니다.
-- excel 파일에서 학생 데이터를 IMPORT 한후 SELECT 실습하기
...
/*
- SELECTION
데이터종에서 조건을 부여하여 조건에 맞는 리스트만 보이기
Recorded의 제한 ( 전체 데이터 => 조건에 맞는 데이터 개수 )
WHERE 절에 조건을 설정하여 데이터를 조회

PK, UNIQUE로 설정된 칼럼을 단일 조건으로 SELECTION 할 때
...
/*
PK, UNIQUE로 설정된 칼럼을 복합조건절로 SELECTION 할 때는 레코드가 1개만 나타날 지라도
이 데이터는 리스트로 취급한다.
*/
SELECT * FROM tbl_student
...
-- PK, UNIQUE로 설정된 칼림이 아닌 경우
-- SELECTION을 수행했을 때 나타나는 레코드가 1개 일지라도 이 데이터는 리스트로 취급한다.
SELECT * FROM tbl_student
WHERE st_name = '독고예준';
...
WHERE 조건절을 사용해서 레코드를 제한하고, 칼럼명을 나열하여 필요한 항목(필드)만 보고자 할 때
칼럼리스트를 제한하기
*/
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
...
-- SELECIONT을 수행할 때는 IN 연산자를 사용하여 구현할 수 있다.
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_num IN( '20001','20003','20006' );

-- 문자열( VARCHAR, nVARCHAR )칼럼일 경우
...
-- LIKE 연산자를 사용하여 조회(검색)을 수행하면 DB에 설정된 모든 INDEX를 무시하고
--      무조건 처음부터 다 뒤지는 연산이 수행된다.
--      이 연산을 Full Text Serch 라고 한다.
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_num LIKE '%7';
