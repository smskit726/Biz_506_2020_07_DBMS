-- 여기는 USER1사용자의 명령공간입니다!
-- USER1은 DBA권한을 부여받았으므로 Table 생성, CRUD 등의 명령을 사용할 수 있다.

/*
- DB에 데이터를 저장하기
DB를 대상으로 업무를 수행할 때 데이터가 있어야만 여러가지 일을 수행할 수 있다.
CRUD 중에서 가장 먼저 수행해야할 명령어 Create형 명령
DML CRUD Create는 DDL CREATE 명령과 구분을 해야 한다.

DDL의 CREATE : 생성이라는 개념이고 운영체제와 밀접한 관련이 있거나, 물리적인 요소가 많다.
               CREATE TABLESPACE, CREATE USER와 같이 저장공간, 사용자 정도 이며 Schema 등을
               생성하는 명령 절차
...
RDBMS(Relationship DataBase Mangement System, 관계형 데이터베이스 시스템)에서
데이터를 추가하려면, 먼저 데이터 저장공간에 대한 정의를 수행해야 한다.
데이터 저장공간을 논리적으로 Entity, 물리적으로 Table 이라고 한다.

TABLE : 표준 SQL(ANSI SQL)에서 데이터를 추가하는 저장공간
        저정할 데이터의 각 필드(칼럼) 항목의 type을 결정
        ex) 학생정보 : 학번, 이름, 학과, 학년, 전화번호, 주소, 나이
        
        저장할 데이터의 최대 길이(크기)를 결정
        데이터의 Key라는 것을 지정
        
PRIMARY KEY : 데이터의 Key중에 가장 중요한 항목
...
VALUES
('10001','홍길동','무역학과',3,'010-1111-1111','서울특별시',33);  -- 데이터들

/*
데이터를 INSERT할 때 혹시 실수로 PK 칼럼에 이미 저장된 데이터를 또 INSERT 하려하면
...
-- 데이터를 추출하여 확인 : 조회(Retrive), 읽기(Read)
SELECT *
FROM tbl_student;
...
-- CRUD에서 Update
/*
tbl_student table에 있는 모든 데이터(모든 행, raw, record)의
st_dept 칼럼의 값을 컴공과로 변경해라
데이터의 개수에 관게없이 명령이 수행되어 버린다.
...
/*
학생이름(st_name) 칼럼에 값이 성춘향인 모든 데이터를 찾아서
그 데이터들의 st_dept 칼럼 값을 음악과로 변경해라

이 명령의 결과로 만약 성춘향 학생의 데이터가 1개밖에 없어서 원하는 결과를 얻었더라도
가급적 사용하지 말아야 하는 명령이다.
어떤 이유로 SELECT 명령을 수행했을 때 성춘향 데이터가 1개밖에 보이지 않았더라도
만에 하나 감줘진(보이지 않는) 영역에 성춘향 데이터가 또 존재를 한다면 이 명령을 수행하는 순간
이 table의 데이터는 무결성을 잃게 된다!
*/
UPDATE tbl_student
SET st_dept = '음악과'
WHERE st_name = '성춘향';
...
/*
학번이 10004, 10007, 10001 인 학생의 학과를 물리학로 변경하고 싶을 때
...
필요가 없어진 데이터는 특정 칼럼에 해당 칼럼에 값을 세팅하여
칼럼 값을 기준으로 필요한 데이터, 필요없는 데이터를 구분한다.
*/

-- table에서 데이터를 삭제하기 CRUD 마지막 Delete 명령
/*
DELETE 명령도 UPDATE 명령과 마찬가지로 반드시 WHERE 절을 동반하는 형태로
명령을 수행해야한다. 그렇지 않으면 최소 1주일간 지옥을 맛보게 된다.
*/
DELETE FROM tbl_student
WHERE st_num = '10004';

-- INSERT, UPDATE, DELETE 명령을 수행 한 후 명령을 확정짓는 명령
-- COMMIT 이후에는 ROLLBACK 으로 해당 명령을 취소할 수 없다.
COMMIT;
...
SELECT st_name ||' 은(는) '|| st_dept || ' 학생 입니다'
FROM tbl_student
WHERE st_dept = '음악과';
...
-- 테이블의 구조 확인 명령
DESCRIBE tbl_student;  -- DESC tbl_student 로 사용하여도 된다.
...
-- SELECTION TYPE 으로 데이터를 제한하여 보기
-- 학과가 컴퓨터공학인 학생들만 보여달라
SELECT *
FROM tbl_student
WHERE st_dept = '컴퓨터공학';

-- 학과가 컴퓨터공학 이고(AND) 3학년인 학생들만 보여달라
SELECT *
FROM tbl_student
WHERE st_dept = '컴퓨터공학' AND st_grade = 3;

-- 학과가 컴퓨터공학 또는(OR) 법학인 학생들만 보여달라
SELECT *
FROM tbl_student
WHERE st_dept = '컴퓨터공학' OR st_dept = '법학';

-- 학과가 컴퓨터공학 또는(OR) 법학인 학생들의 정보 중 이름과 학과만 나열하라
...
-- 부등호(범위 연산자)를 포함하여 값을 조회할 수 있다.
-- 일반 프로그래밍 코드에서는 문자일 경우 사용불가
...
-- WHERE st_num >= '2010-01-01' AND st_num <='2020-10-31';
